...
export const useFunnelLogic = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const parsed = JSON.parse(saved) as SavedFunnel;
  ...
  const saveToStorage = useCallback(
    (currentNodes: Node[], currentEdges: Edge[], currentCounters: Record<NodeType, number>) => {
  ...
  const onConnect: OnConnect = useCallback(
    (params) => {
      ...
      if (sourceData?.type === NodeType.SALES) {
        const outgoers = getOutgoers(sourceNode, nodes, edges);
        if (outgoers.length >= 1) {
          showToast.error('Sales Page can only have one outgoing connection.');
          return;
        }
      }
    ...
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const parsed = JSON.parse(content) as SavedFunnel;
  ...
  useEffect(() => {
    ...
    nodes.forEach((node) => {
      ...
      if (type !== NodeType.SALES && incomingEdges.length === 0) {
        issues.push({
          nodeId: node.id,
          message: `${nodeData.label} is unreachable (no incoming connections).`,
          severity: 'warning',
        });
      ...
      if (type !== NodeType.THANK_YOU && outgoingEdges.length === 0) {
        issues.push({
          nodeId: node.id,
          message: `${nodeData.label} is a dead end (no outgoing connections).`,
          severity: 'warning',
        });
        nodeIdsWithIssues.add(node.id);
      }
    ...
    setNodes((currentNodes) => {
      ...
      const newNodes = currentNodes.map((n) => {
        ...
        const currentNodeData = n.data as FunnelNodeData;
        if (!!currentNodeData.isInvalid !== isInvalid) {
          hasChanges = true;
          return { ...n, data: { ...currentNodeData, isInvalid } };
        }
        return n;
      });
